...
def cv_train(args, model, dataloader, logger, setting, k):
    ...
    for epoch in tqdm.tqdm(range(args.epochs)):
        ...
        for idx, data in enumerate(dataloader[f'train_{k}_dataloader']):
            if args.model == 'CNN_FM':
                x, y = [data['user_isbn_vector'].to(args.device), data['img_vector'].to(args.device)], data['label'].to(args.device)
            elif args.model == 'DeepCoNN':
                x, y = [data['user_isbn_vector'].to(args.device), data['user_summary_merge_vector'].to(args.device), data['item_summary_vector'].to(args.device)], data['label'].to(args.device)
        ...
        valid_loss = cv_valid(args, model, dataloader, loss_fn, k)
        ...
        if minimum_loss > valid_loss:
            minimum_loss = valid_loss
            os.makedirs(args.saved_model_path, exist_ok=True)
            torch.save(model.state_dict(), f'{args.saved_model_path}/{setting.save_time}_{args.model}_model_{k}.pt')
    logger.close()
    return model


def cv_valid(args, model, dataloader, loss_fn, k):
    model.eval()
    total_loss = 0
    batch = 0

    for idx, data in enumerate(dataloader[f'valid_{k}_dataloader']):
        if args.model == 'CNN_FM':
            x, y = [data['user_isbn_vector'].to(args.device), data['img_vector'].to(args.device)], data['label'].to(args.device)
        elif args.model == 'DeepCoNN':
            x, y = [data['user_isbn_vector'].to(args.device), data['user_summary_merge_vector'].to(args.device), data['item_summary_vector'].to(args.device)], data['label'].to(args.device)
...
def cv_test(args, models, dataloader, setting):
    ...
    predicts = np.mean(predict_list, axis=1)
    
    return predicts


def retrain(args, model, dataloader, logger, setting):
    ...
    for epoch in tqdm.tqdm(range(args.epochs)):
        ...
        for idx, data in enumerate(dataloader[f'retrain_dataloader']):
            if args.model == 'CNN_FM':
                x, y = [data['user_isbn_vector'].to(args.device), data['img_vector'].to(args.device)], data['label'].to(args.device)
            elif args.model == 'DeepCoNN':
                x, y = [data['user_isbn_vector'].to(args.device), data['user_summary_merge_vector'].to(args.device), data['item_summary_vector'].to(args.device)], data['label'].to(args.device)
        ...
        print(f'Epoch: {epoch+1}, Train_loss: {total_loss/batch:.3f}')
        logger.log(epoch=epoch+1, train_loss=total_loss/batch, valid_loss=0)
        torch.save(model.state_dict(), f'{args.saved_model_path}/{setting.save_time}_{args.model}_model.pt')
